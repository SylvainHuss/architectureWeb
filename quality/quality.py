...
try:
    ...
    try:
        ...
        r = requests.get("http://172.16.0.6:3000/api/radios/" + str(id) + "/up", timeout=10, verify=False, stream=True)
        print (radioName + " is up")

    except:
        requests.get("http://172.16.0.6:3000/api/radios/" + str(id) + "/down", timeout=10, verify=False, stream=True)
